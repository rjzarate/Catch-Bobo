...
execute as @e[type=armor_stand,tag=jett] at @s unless block ~ ~ ~ #cb:dashables run kill @s
execute as @e[type=armor_stand,tag=jett] at @s unless block ~ ~1 ~ #cb:dashables run kill @s

tag @s add dashing

schedule function cb:game/during/player/skills/hunters/jett/dash/dashing/1 1t

schedule function cb:game/during/player/skills/hunters/jett/dash/dashing/kill 6t