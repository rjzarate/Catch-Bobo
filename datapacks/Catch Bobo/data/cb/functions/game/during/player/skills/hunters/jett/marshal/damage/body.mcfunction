...
execute at @p[tag=shot] run summon bat ~ 400 ~ {Silent:1b,PersistenceRequired:1b,NoAI:1b,Health:1f,Tags:["temp"],CustomName:'{"text":"Filler"}',ArmorItems:[{},{},{},{id:"minecraft:leather_helmet",Count:1b,tag:{Enchantments:[{id:"minecraft:thorns",lvl:15s}]}}],ArmorDropChances:[0.085F,0.085F,0.085F,0.000F],ActiveEffects:[{Id:14b,Amplifier:0b,Duration:20000000,ShowParticles:0b}]}

execute at @p[tag=shot] run summon arrow ~ 401.3 ~ {Owner:[I;0,0,0,0],Tags:["temp","marshal"]}

execute at @s as @e[type=arrow,tag=temp,tag=marshal] run data modify entity @s Owner set from entity @e[distance=0..1,limit=1,sort=nearest] UUID
...
execute as @e[type=arrow,tag=temp,tag=player] run data modify entity @s Owner set from entity @p[tag=shot] UUID