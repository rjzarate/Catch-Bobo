...
execute if predicate orangutan:bobo/starfruit_swapper if score @s starfruitSwapperTangerineTeleporter matches 1.. unless entity @a[tag=teleporter] if predicate general:on_ground at @s unless block ~ ~ ~ minecraft:lava run function orangutan:bobo/starfruit_swapper/activate
execute if predicate orangutan:bobo/tangerine_teleporter if score @s starfruitSwapperTangerineTeleporter matches 1.. run function orangutan:bobo/tangerine_teleporter/activate