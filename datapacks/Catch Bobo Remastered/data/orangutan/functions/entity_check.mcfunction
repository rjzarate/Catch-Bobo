...
execute if score .number hotPotatoPlanterFuse > .fuse hotPotatoPlanterFuse as @e[type=minecraft:item,tag=hotPotato] at @s if entity @e[type=#general:hunters,tag=hunter,distance=..1,tag=!spectator] unless entity @a[tag=timeStopper] run function orangutan:bobo/hot_potato_planter/picked_up
...
execute if score .number hotPotatoPlanterFuse matches 20 unless entity @a[tag=timeStopper] as @a[tag=hunter,gamemode=!spectator,tag=!adwared] unless score @s hotPotatoDropped matches 1.. at @s run playsound minecraft:entity.tnt.primed player @a ~ ~ ~ 1 2
...
execute as @e[type=minecraft:slime,tag=noxiousTrap,tag=finished,tag=!timeStoppee] if score @s noxiousTrapDuration < .armTime noxiousTrapStats at @s if entity @e[tag=hunter,type=#general:hunters,tag=!spectator,distance=..1] run function orangutan:teemo/noxious_trap/stepped_on
...
#stunned players
	...
	execute as @a[tag=hunter,gamemode=!spectator,tag=impostorStun] run function orangutan:impostor/kill/stun
#turtle vents
