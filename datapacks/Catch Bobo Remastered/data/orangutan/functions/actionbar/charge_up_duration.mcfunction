...
##impostor only
	#not near immobilized player
		execute if predicate orangutan:impostor/report_body unless score chargeUp actionbarCD matches 1.. unless score duration actionbarCD matches 1.. if score int actionbarCD matches ..0 if score dec actionbarCD matches ..0 at @s unless entity @a[tag=hunter,gamemode=!spectator,distance=..4,tag=impostorStun] run title @s actionbar ["",{"text":"Ready ","bold":true,"color":"yellow"},{"text":"| ","bold":true},{"text":"CD: ","bold":true,"color":"yellow"},{"text":"\u2b24 ","color":"dark_green"},{"text":"(0.0)","italic":true,"color":"#D4D439"}]
	#1 or less hunters alive
		execute if predicate orangutan:impostor/sabotage store result score #aliveHunters temp if entity @a[tag=hunter,gamemode=!spectator]
