...
##impostor only
	#player not in range
	execute if predicate orangutan:impostor/kill if score int actionbarCD matches ..0 if score dec actionbarCD matches ..0 at @s unless entity @a[team=Hunters,gamemode=!spectator,distance=..4,tag=!impostorStun,tag=!invincible] run title @s actionbar ["",{"text":"Ready ","bold":true,"color":"yellow"},{"text":"| ","bold":true},{"text":"CD: ","bold":true,"color":"yellow"},{"text":"\u2b24 ","color":"dark_green"},{"text":"(0.0)","italic":true,"color":"#D4D439"}]
	#vent not in range
