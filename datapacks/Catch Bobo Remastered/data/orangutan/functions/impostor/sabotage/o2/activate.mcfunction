...
#ocelot summon
	execute as @a[team=Hunters,gamemode=!spectator,limit=2,sort=furthest] at @s run summon minecraft:ocelot ~ ~ ~ {Glowing:1b,Team:"colorGreen",Health:5f,Tags:["sabotage","o2","timeStoppable","new"],Attributes:[{Name:generic.max_health,Base:5},{Name:generic.movement_speed,Base:0.30000001192092896d}],CustomName:'{"text":"O2","bold":true}'}
...
title @a[team=Hunters,gamemode=!spectator] subtitle ["",{"text":"Kill both green ocelots to fix oxygen levels.","bold":false,"italic":true,"color":"white"}]
