...
title @a[tag=hunter,gamemode=!spectator] title ["",{"text":"FIX LIGHTS","bold":true,"italic":false,"color":"red"}]
title @a[tag=hunter,gamemode=!spectator] subtitle ["",{"text":"Kill both gray ocelots to fix lights.","bold":false,"italic":true,"color":"white"}]
tellraw @a[tag=hunter,gamemode=!spectator] ["",{"text":"FIX LIGHTS\n","bold":true,"italic":false,"color":"red"},{"text":"Kill both gray ocelots to fix lights.\nFailure results in darkness; ocelots can still be killed to remove the darkness effect.","bold":false,"italic":true,"color":"white"}] 
