...
execute at @s run tp @s @p[team=Hunters,gamemode=!spectator,tag=!impostorStun,tag=!displayOnly,tag=!invincible]
execute at @s run playsound minecraft:entity.turtle.egg_crack player @a ~ ~ ~ 1 0.75

#specific kits
	execute at @s as @p[team=Hunters,gamemode=!spectator,tag=!impostorStun,tag=!displayOnly,tag=!invincible] if entity @s[tag=vaccinator] run clear @s minecraft:tipped_arrow{Tags:["syringe"]}
	execute at @s as @p[team=Hunters,gamemode=!spectator,tag=!impostorStun,tag=!displayOnly,tag=!invincible] if entity @s[tag=bobaFett] run item replace entity @s armor.chest with elytra{display:{Name:'{"text":"Jetpack","color":"dark_red","bold":true,"italic":false}',color:3950130},Unbreakable:0b,Enchantments:[{id:"minecraft:binding_curse",lvl:1s}],AttributeModifiers:[{AttributeName:"generic.max_health",Name:"generic.max_health",Amount:0,Operation:0,UUID:[I;1234567890,-1234567890,1234567890,-1234567890]}],HideFlags:7,Damage:432} 1


execute at @s as @p[team=Hunters,gamemode=!spectator,tag=!impostorStun,tag=!displayOnly,tag=!invincible] run tag @s add impostorStun
