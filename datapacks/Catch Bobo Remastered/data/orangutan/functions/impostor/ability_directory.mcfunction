execute if predicate orangutan:impostor/kill at @s if entity @a[team=Hunters,gamemode=!spectator,distance=..4,tag=!impostorStun,tag=!invincible] unless score @s killCD matches 1.. run function orangutan:impostor/kill/activate
