...
#successful shot
	execute if score @s marshalEquip >= .equipTime20t marshalStats if score @s marshalScope >= .scopeTime20t marshalStats run function hunter:jett/marshal/fire

#sound effects on moving
	execute if score #consumeAmmo marshalRightClick matches 1 at @s run playsound minecraft:block.note_block.hat player @a ~ ~ ~ 10 2
	execute if score #consumeAmmo marshalRightClick matches 1 at @s anchored eyes run particle minecraft:crit ^ ^ ^ 0.2 0.2 0.2 1 5



###FOR GROUNDED AND MOVING REQUIREMENT
#successful shot
