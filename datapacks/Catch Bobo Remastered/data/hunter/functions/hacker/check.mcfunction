...
execute if entity @s[advancements={hunter:hacker=true},tag=!timeStoppee,tag=!impostorStun,tag=!displayOnly] if score .rightClick enderEye matches 0 run function hunter:hacker/ability_directory
...
#trojan horse crossbow activation
	...
	execute if entity @s[advancements={hunter:hacker/crossbow=true},tag=!impostorStun,tag=!displayOnly] if score @s battery >= .trojanHorseBatteryUsage hackerStats run function hunter:hacker/trojan_horse/activate
